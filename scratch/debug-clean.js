...
const { preprocessWebVTT } = require("../lib/pipeline/stage0-preprocess"); 
...
async function test() {
  ...
  console.log("Clean Transcript Length:", clean.length);
  ...
  const result = await model.generateContent(`Segment this transcript into chapters. Ensure the entire timeline from the very first line to the very last line is covered:\n\n${clean}`);
  console.log("GEMINI CLEAN OUTPUT:");
